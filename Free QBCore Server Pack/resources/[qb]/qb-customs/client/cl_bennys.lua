...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'mechanic' then
            ...
            --QBCore.Functions.Notify('Einige Käufe sind derzeit nicht verfügbar. Bitte finden Sie einen Mechaniker.')
			exports['okokNotify']:Alert("Custom",  "Einige Käufe sind derzeit nicht verfügbar. Bitte finde einen Mechaniker.", 5000, 'info')
        end
        ...
        if not canEnter then
			...
			exports['okokNotify']:Alert("Custom", "Hier kannst du nichts machen!", 5000, 'info')
...
RegisterNetEvent("qb-customs:client:purchaseSuccessful", function()
	...
	exports['okokNotify']:Alert("Custom", "Kauf erfolgreich", 5000, 'success')
...
RegisterNetEvent("qb-customs:client:purchaseFailed", function()
	...
	exports['okokNotify']:Alert("Custom", "Nicht genug Geld", 5000, 'error')
end)
