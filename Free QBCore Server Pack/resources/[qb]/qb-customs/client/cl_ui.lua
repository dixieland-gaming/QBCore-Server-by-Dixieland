...
function InitiateMenus(isMotorcycle, vehicleHealth, categories, welcomeLabel)
    ...
    --#[Main Menu]#--
    createMenu("mainMenu", welcomeLabel, "Wähle eine Kategorie")
    ...
    for _, v in ipairs(vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if v.id == 11 or v.id == 12 or v.id == 13 or v.id == 15 or v.id == 16 then --Performance Upgrades
                ...
                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category, "Wählen Sie ein Upgrade")
            ...
            elseif v.id == 18 then
                ...
                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category .. " Anpassung", "Turbo aktivieren oder deaktivieren")
                ...
                updateItem2Text(v.category:gsub("%s+", "") .. "Menu", currentTurboState, "Installiert")

                finishPopulatingMenu(v.category:gsub("%s+", "") .. "Menu")
            else
                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category .. " Anpassung", "Wähle einen Mod")

                for _, n in pairs(validMods) do
                    populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. vehicleCustomisationPrices.cosmetics.price)

                    if currentMod == n.id then
                        updateItem2Text(v.category:gsub("%s+", "") .. "Menu", n.id, "Installiert")
                    end
                end
    ...
    --#[Respray Menu]#--
    createMenu("ResprayMenu", "Lackieren", "Wähle eine Farbkategorie")
    ...
    --#[Respray Types]#--
    createMenu("ResprayTypeMenu", "Lackierungs Typ", "Wähle einen Farbtyp")
    ...
    --#[Respray Colours]#--
    for _, v in ipairs(vehicleResprayOptions) do
        createMenu(v.category .. "Menu", v.category .. " Farben", "Wähle eine Farbe")
    ...
    createMenu("WheelsMenu", "Radkategorien", "Wähle eine Kategorie")
        ...
        elseif v.id ~= 20 then
            if isMotorcycle then
                if v.id == 6 then --Motorcycle Wheels
                    ...
                    createMenu(v.category .. "Menu", v.category .. " Räder", "Wählen Sie ein Rad")
    ...
    createMenu("TyreSmokeMenu", "Reifenrauch-Anpassung", "Wähle eine Farbe")
    ...
    createMenu("WindowTintMenu", "Anpassung der Fenstertönung", "Wähle eine Tönung")
    ...
    if livCount > 0 then
        ...
        createMenu("OldLiveryMenu", "Anpassung der alten Lackierung", "Wähle eine Lackierung")
    ...
    createMenu("PlateIndexMenu", "Plattenfarbe", "Wähle einen Stil")
    local plateTypes = {
        "Blue on White #1",
        "Yellow on Black",
        "Yellow on Blue",
    ...
    --#[Neons Menu]#--
    createMenu("NeonsMenu", "Neon Anpassung", "Wähle eine Kategorie")
    ...
    createMenu("NeonColoursMenu", "Neonfarben", "Wähle eine Farbe")
    ...
    --#[Xenons Menu]#--
    createMenu("XenonsMenu", "Xenon Anpassung", "Wähle eine Kategorie")
    ...
    createMenu("XenonColoursMenu", "Xenon Farben", "Wähle eine Farbe")
                ...
                if currentMenu == "repairMenu" then
                    if AttemptPurchase("repair") then
                        ...
                        if not repairOnly then
							...
							exports['okokNotify']:Alert("Custom", "Dein Fahrzeug wurde repariert!", 5000, 'info')
