...
RegisterNetEvent('qb-customs:server:attemptPurchase', function(type, upgradeLevel, location)
    ...
    if paidBySociety then
        if exports['qb-management']:GetAccount(job) >= price then
			...
			TriggerClientEvent('okokNotify:Alert', source, "Custom", "Ihre Arbeitsgesellschaft kann das nicht bezahlen. Stattdessen werden dir Gebühren in Rechnung gestellt.", 5000, 'info') 
        end
    end
    if balance >= price or paidBySociety then
        ...
        if jobRestricted and job ~= 'mechanic' then
           exports['qb-management']:AddMoney("police", 150) 
	       exports['qb-management']:AddMoney("ambulance", 100)
	       exports['qb-management']:AddMoney("mechanic", 50)	
        end
